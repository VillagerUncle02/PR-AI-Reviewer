...
- 校验规则：
  - 符合严格 SemVer 2.0.0（`^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-((?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*)(?:\.(?:0|[1-9]\d*|\d*[a-zA-Z-][0-9a-zA-Z-]*))*))?(?:\+([0-9a-zA-Z-]+(?:\.[0-9a-zA-Z-]+)*))?$`，与 scripts 三脚本一致：拒绝前导零、支持 pre-release 与 build metadata），不允许 `v` 前缀进入 `version`；
