...
- Q: 评论范围校验需要额外的只读 GitHub 查询吗？ → A: 不需要；只做一次提交，越界评论由 GitHub 原子性拒绝后映射为明确失败原因。
...
- 评论指向的文件或行不在目标 PR 的 file change 范围内：整次调用判为失败并返回明确原因；不得忽略该评论而部分提交；越界判定以提交结果为准，工具不做额外只读查询。
...
- **FR-004**：任何评论的文件路径或行位置超出目标 PR 的 file change 范围时，提交 MUST 整体失败并返回明确原因；该校验 MUST 以提交结果为准，工具 MUST NOT 通过额外的只读查询（如读取 PR 状态或改动文件列表）进行前置校验。
