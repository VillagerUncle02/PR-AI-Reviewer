---
description: Run the full implementation loop (branch -> tasks -> gates -> AI review/fix -> commit -> CI -> PR -> AI PR review -> next feature) with human gates preserved
scripts:
...
其中 `{SCRIPT}` 是注册后的脚本路径（通常为 `.specify/extensions/implement-loop/scripts/powershell/load-config.ps1`，`specify extension add` 后自动改写）。若用户传了 `--feature <dir>`，改为运行 `{SCRIPT} -Feature "<dir>"`（等价于命令内联的 `-Feature` 参数）。解析输出 JSON，得到以下关键字段（全部为绝对路径或已解析值）：
